...
public class ValueRange extends ValueSet{
    ...
    public ValueRange(DynamicForm masterForm) throws RuntimeException{
        this.rangeLimits = Arrays.stream(NumberInputLayout.RangeItem.names)
                ...
                .map(item -> (Float) item.getValue())
                .filter(Objects::nonNull)
                .toArray(Float[]::new);
        if(this.rangeLimits.length < 3){
            throw new RuntimeException("Cannot create value range: please ensure provided form has non-empty fields "
                    + "with names " + Arrays.toString(NumberInputLayout.RangeItem.names));
        }
